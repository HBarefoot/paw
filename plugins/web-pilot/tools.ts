...
export function createWebPilotTools(getPage: (sessionId?: string) => Promise<Page>): ToolDefinition[] {
  return [
    {
      ...
      input_schema: {
        ...
        properties: {
          ...
          session_id: { type: "string", description: "Session ID for page isolation (optional)" },
        },
        required: ["url"],
      },
      plugin: "web-pilot",
    ...
    },
    {
      ...
      description: "Get the visible text content of the current page or a specific selector",
      input_schema: {
        type: "object",
        properties: {
          selector: { type: "string", description: "CSS selector to get text from (defaults to body)" },
      ...
      handler: async (input) => {
        ...
        return { content: trimmed + (text.length > 4000 ? "\n...[truncated]" : "") };
      },
    },
    {
      name: "browser_click",
    ...
    },
    {
      ...
      handler: async (input) => {
        ...
        return { content: `Screenshot captured (${buffer.byteLength} bytes). Base64: ${base64.slice(0, 200)}...` };
      },
    },
    {
