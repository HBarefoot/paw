...
export default class SlackPlugin implements ChannelPlugin {
  ...
  async register(ctx: PluginContext): Promise<void> {
    ...
    this.app.message(async ({ message, say }) => {
      ...
      ctx.logger.info("Slack message received", { user: inbound.user.id, channel: message.channel });
      await ctx.bus.emit("message:inbound", inbound);
    });

    // Listen for outbound messages to post replies
    this.unsubOutbound = ctx.bus.on("message:outbound", async (msg: OutboundMessage) => {
      ...
      try {
        ...
        ctx.logger.error("Failed to send Slack reply", { error: String(err) });
      }
    });
  }
