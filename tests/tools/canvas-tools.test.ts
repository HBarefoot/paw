...
describe("canvas tools", () => {
  ...
  test("canvas_write creates a file", async () => {
    const result = await getTool("canvas_write").handler({ path: "index.html", content: "<h1>Hello</h1>" });
    expect(result.is_error).toBeUndefined();
    const parsed = JSON.parse(result.content);
    expect(parsed.written).toBe(true);
    expect(parsed.path).toBe("index.html");
    expect(readFileSync(join(CANVAS_ROOT, "index.html"), "utf-8")).toBe("<h1>Hello</h1>");
  });

  test("canvas_write creates parent directories", async () => {
    const result = await getTool("canvas_write").handler({ path: "css/deep/style.css", content: "body { color: red; }" });
    expect(result.is_error).toBeUndefined();
    expect(existsSync(join(CANVAS_ROOT, "css/deep/style.css"))).toBe(true);
  });

  test("canvas_write rejects path traversal", async () => {
    const result = await getTool("canvas_write").handler({ path: "../../../etc/evil", content: "bad" });
  ...
  test("canvas_read rejects path traversal", async () => {
    const result = await getTool("canvas_read").handler({ path: "../../../etc/passwd" });
    expect(result.is_error).toBe(true);
    expect(result.content).toContain("outside canvas root");
  });
