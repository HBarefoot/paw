...
describe("file tools", () => {
  ...
  beforeEach(() => {
    ...
    tools = createFileTools({ workspacePath: WORKSPACE, maxFileSize: 1_048_576, maxOutputLength: 10_000 });
  });
  ...
  test("file_read with offset and limit", async () => {
    const result = await getTool("file_read").handler({ path: "hello.txt", offset: 1, limit: 1 });
    expect(result.content).toBe("Line 2");
  });

  test("file_read rejects path traversal", async () => {
    const result = await getTool("file_read").handler({ path: "../../../etc/passwd" });
  ...
  test("file_write creates a file", async () => {
    const result = await getTool("file_write").handler({ path: "new.txt", content: "hello" });
    expect(result.is_error).toBeUndefined();
  ...
  test("file_write appends to a file", async () => {
    await getTool("file_write").handler({ path: "hello.txt", content: "\nLine 4", append: true });
    const result = await getTool("file_read").handler({ path: "hello.txt" });
  ...
  test("file_list recursive", async () => {
    const result = await getTool("file_list").handler({ path: ".", recursive: true });
