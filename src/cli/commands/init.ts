...
export async function initCommand(): Promise<void> {
	...
	try {
		...
		if (
			overrides.ai &&
			typeof overrides.ai === "object" &&
			"model" in overrides.ai
		) {
			...
			(overrides.ai as Record<string, unknown>).model =
...
async function chooseProvider(
...
): Promise<"claude" | "ollama" | "openai" | "gemini"> {
	...
	const providers: Array<"claude" | "openai" | "gemini" | "ollama"> = [
		"claude",
		"openai",
		"gemini",
		"ollama",
		...
		if (res.ok) {
			...
			if (models.length > 0) {
				...
				const hasModel = models.some((m: string) =>
					m.startsWith(creds.ollama!.model),
				);
				if (!hasModel) {
					console.log(
						`  ⚠ Model "${creds.ollama.model}" not found. You may need to pull it: ollama pull ${creds.ollama.model}`,
					);
				}
			} else {
			...
			console.log(
				`  ⚠ Ollama responded with HTTP ${res.status}. Check your URL.`,
			);
		}
	} catch {
		console.log(
			`  ⚠ Could not reach Ollama at ${url}. Make sure it's running.`,
		);
	}
...
async function setupOpenAI(creds: StoredCredentials): Promise<void> {
	...
	if (!apiKey) {
		console.log(
			"  ⚠ No API key entered. Set it later with: OPENAI_API_KEY env var",
		);
		creds.openai = { apiKey: "" };
		return;
	}
...
async function setupGemini(creds: StoredCredentials): Promise<void> {
	...
	if (!apiKey) {
		console.log(
			"  ⚠ No API key entered. Set it later with: GEMINI_API_KEY env var",
		);
		creds.gemini = { apiKey: "" };
		return;
	}
