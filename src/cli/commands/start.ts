...
import { getAnthropicCredentials, getStoredProvider, getOllamaConfig } from "../../auth/credential-store.js";
...
export async function startCommand(): Promise<void> {
  ...
  if (provider === "ollama") {
    ...
    const method = creds.method === "oauth" ? "OAuth (Max/Pro subscription)" : "API key";
    console.log(`\n  Using: Claude — ${method}`);
  }
  ...
  if (legacyDb !== config.store.dbPath && existsSync(legacyDb) && !existsSync(config.store.dbPath)) {
    renameSync(legacyDb, config.store.dbPath);
    // Also migrate WAL/SHM files if present
    for (const suffix of ["-wal", "-shm"]) {
      if (existsSync(legacyDb + suffix)) renameSync(legacyDb + suffix, config.store.dbPath + suffix);
    }
  }
