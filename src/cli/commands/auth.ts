...
import { saveConfigOverrides } from "../../config/writer.js";
...
async function authLogin(): Promise<void> {
	...
	try {
		...
		const providerDefaults: Record<string, { model: string }> = {
			claude: { model: defaults.ai.model },
			openai: { model: defaults.openai.model },
			gemini: { model: defaults.gemini.model },
			ollama: { model: defaults.ollama.model },
		};

		if (choice === 0) {
			await loginAnthropic(creds);
			creds.provider = "claude";
		} else if (choice === 1) {
			...
			await loginOllama(creds);
			creds.provider = "ollama";
		} else if (choice === 4) {
			await loginSlack(creds);
			// Slack is a plugin, not an AI provider — don't change provider
		}

		saveCredentials(creds);

		// Reset ai.model in config file to match the new provider's default
		if (choice !== 4 && creds.provider) {
			const defaultModel = providerDefaults[creds.provider]?.model;
			if (defaultModel) {
				saveConfigOverrides({ ai: { model: defaultModel } });
			}
		}

		if (choice !== 4) {
			console.log(`\n  ✓ Credentials saved. Active provider: ${creds.provider}`);
		}
	...
	if (method === 0) {
		...
		if (input && input.startsWith("sk-ant-")) {
			...
			if (token) {
				...
				console.log(
					"  ✗ No token found. Make sure `claude setup-token` completed successfully.",
				);
			}
		}
	} else {
			...
			const openCmd =
				process.platform === "darwin"
					...
					: process.platform === "win32"
						? "start"
						: "xdg-open";
			spawn(openCmd, ["https://console.anthropic.com/settings/keys"], {
				stdio: "ignore",
				detached: true,
			}).unref();
...
async function loginOpenAI(creds: StoredCredentials): Promise<void> {
		...
		const openCmd =
			process.platform === "darwin"
				...
				: process.platform === "win32"
					? "start"
					: "xdg-open";
		spawn(openCmd, ["https://platform.openai.com/api-keys"], {
			stdio: "ignore",
			detached: true,
		}).unref();
		console.log("  (Opened in your browser)\n");
	...
	const wantCustomBase = await promptConfirm(
		"Use a custom base URL? (for Azure, Together, etc.)",
		false,
	);
		...
		const openCmd =
			process.platform === "darwin"
				...
				: process.platform === "win32"
					? "start"
					: "xdg-open";
		spawn(openCmd, ["https://aistudio.google.com/apikey"], {
			stdio: "ignore",
			detached: true,
		}).unref();
		console.log("  (Opened in your browser)\n");
...
async function authLogout(): Promise<void> {
	...
	try {
		...
		if (target === undefined) {
			...
			console.log(
				`  ✓ ${target.charAt(0).toUpperCase() + target.slice(1)} credentials cleared`,
			);
		}
		console.log();
	} finally {
...
function authStatus(): void {
	...
	if (creds.anthropic?.method === "oauth" && creds.anthropic.accessToken) {
		...
		console.log(
			"  Anthropic: ✓ Claude CLI token detected (run `paw init` to import)",
		);
	} else {
	...
	if (provider === "ollama") {
		if (creds.ollama?.baseUrl) {
			console.log(
				`  Ollama:    ✓ ${creds.ollama.model} @ ${creds.ollama.baseUrl}`,
			);
