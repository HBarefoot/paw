...
export function createFileTools(config: FileToolsConfig): ToolDefinition[] {
  ...
  const fileRead: ToolDefinition = {
    ...
    handler: async (input): Promise<ToolResult> => {
      ...
      if (!filePath) return { content: "Error: path is outside workspace", is_error: true };

      if (!existsSync(filePath)) return { content: `Error: file not found: ${input.path}`, is_error: true };

      const stat = statSync(filePath);
      if (stat.isDirectory()) return { content: "Error: path is a directory, use file_list instead", is_error: true };
      if (stat.size > config.maxFileSize) {
        return { content: `Error: file too large (${stat.size} bytes, max ${config.maxFileSize})`, is_error: true };
      }
  ...
  const fileWrite: ToolDefinition = {
    name: "file_write",
    description: "Write or append content to a file within the workspace directory.",
    plugin: "kernel",
    input_schema: {
      type: "object",
      properties: {
    ...
    handler: async (input): Promise<ToolResult> => {
      ...
      if (content.length > config.maxFileSize) {
        return { content: `Error: content too large (${content.length} bytes, max ${config.maxFileSize})`, is_error: true };
      }
  ...
  const fileList: ToolDefinition = {
    ...
    handler: async (input): Promise<ToolResult> => {
      ...
      if (!dirPath) return { content: "Error: path is outside workspace", is_error: true };

      if (!existsSync(dirPath)) return { content: `Error: directory not found: ${input.path || "."}`, is_error: true };
      if (!statSync(dirPath).isDirectory()) return { content: "Error: path is not a directory", is_error: true };
