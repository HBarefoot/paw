...
export function createExecTools(config: ExecToolsConfig): ToolDefinition[] {
	const execCommand: ToolDefinition = {
		name: "exec_command",
		description:
			"Execute a command within the workspace. Provide the command and arguments. Shell operators (pipes, redirects, semicolons, &&) are not allowed — use separate tool calls instead.",
		plugin: "kernel",
		input_schema: {
			type: "object",
			properties: {
				command: {
					type: "string",
					description: "The command to execute (e.g. 'ls -la src')",
				},
				cwd: {
					type: "string",
					description:
				...
				timeout_ms: {
					type: "number",
					description: `Timeout in ms (default: ${config.execTimeout})`,
				},
			},
			required: ["command"],
		},
		handler: async (input): Promise<ToolResult> => {
			...
			if (SHELL_METACHAR_RE.test(command)) {
				return {
					content:
						"Error: shell operators (;, &, |, `, $, etc.) are not allowed. Use separate exec_command calls for multiple commands.",
					is_error: true,
				};
			}
			...
			if (config.allowedCommands && config.allowedCommands.length > 0) {
				if (!config.allowedCommands.includes(executable)) {
					return {
						content: `Error: command '${executable}' is not in the allowed list`,
						is_error: true,
					};
				}
			}
			...
			if (cwdRel.startsWith("..")) {
				return {
					content: "Error: working directory must be within the workspace",
					is_error: true,
				};
			}
			...
			try {
				...
				if (output.length > config.maxOutputLength) {
					output =
						output.slice(0, config.maxOutputLength) + "\n... (truncated)";
				}
