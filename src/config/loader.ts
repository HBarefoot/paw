...
import { getAnthropicCredentials, getSlackCredentials, getStoredProvider, getOllamaConfig, getOpenAICredentials, getGeminiCredentials } from "../auth/credential-store.js";
...
function resolvedCredentials(): Record<string, unknown> {
  ...
  return {
    ...
    ai: {
      ...
      maxTokens: env.PAW_AI_MAX_TOKENS ? Number(env.PAW_AI_MAX_TOKENS) : undefined,
      maxToolRoundtrips: env.PAW_AI_MAX_ROUNDTRIPS ? Number(env.PAW_AI_MAX_ROUNDTRIPS) : undefined,
    },
    ollama: {
    ...
    slack: slackCreds
      ...
      : undefined,
    webPilot: {
      headless: env.PAW_WEBPILOT_HEADLESS !== undefined ? env.PAW_WEBPILOT_HEADLESS !== "false" : undefined,
      maxPages: env.PAW_WEBPILOT_MAX_PAGES ? Number(env.PAW_WEBPILOT_MAX_PAGES) : undefined,
    },
    web: {
      enabled: env.PAW_WEB_ENABLED !== undefined ? env.PAW_WEB_ENABLED === "true" : undefined,
...
export function loadConfig(overrides?: Partial<PawConfig>): PawConfig {
  let merged = deepMerge(defaults as unknown as Record<string, unknown>, resolvedCredentials());
