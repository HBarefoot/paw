...
export function parseCSV(buffer: Buffer, maxRows = 500): string {
  ...
  for (let i = 0; i < text.length; i++) {
    if (text[i] === "\n") { count++; if (count === maxRows) { lastIdx = i; break; } }
  }
  if (count >= maxRows) {
    const totalLines = text.split("\n").length;
    return text.slice(0, lastIdx) + "\n... (truncated, " + totalLines + " total rows)";
  }
  return text;
}

export async function parseExcel(buffer: Buffer, maxRows = 500): Promise<string> {
...
export async function parseUploadedFiles(
  ...
  for (const file of files) {
    ...
    if (ext === "csv") {
      ...
      try { textContent = await parseExcel(buffer); }
      catch { textContent = "(Failed to parse Excel file)"; }
    }

    if (textContent) {
      attachments.push({
        type: "text", data: Buffer.from(textContent, "utf-8"),
        mimeType: "text/plain", name: file.name,
      });
    }
  }
  return attachments;
