...
// Anchor smooth-scroll + cross-page link routing + form→fetch shim. The
// canvas/app pages render in a null-origin sandbox (preview) / tight CSP (app),
// so native navigations to another canvas page fail (an opaque origin never
// matches the CSP 'self' token → ERR_CONNECTION_REFUSED / blocked frame). This
// rewires them: in-page #anchors smooth-scroll, links to another
// /api/canvas/preview/* page are handed to the parent console to open in the
// canvas tab system (navigate-in-place, or a new tab on meta/ctrl/middle-click
// or target=_blank), external http(s) links are handed up to open in a new
// browser tab, and forms posting to /api/forms/* submit via fetch and toast.
const CANVAS_RUNTIME = `<script>(function(){document.addEventListener("click",function(e){var a=e.target.closest&&e.target.closest("a[href]");if(!a)return;var href=a.getAttribute("href")||"";if(href.charAt(0)==="#"){e.preventDefault();var id=href.slice(1);if(!id){window.scrollTo({top:0,behavior:"smooth"});return;}var el=document.getElementById(id)||document.querySelector('[name="'+id+'"]');if(el)el.scrollIntoView({behavior:"smooth"});return;}var url;try{url=new URL(a.href);}catch(err){return;}if(url.protocol!=="http:"&&url.protocol!=="https:")return;var PFX="/api/canvas/preview/";if(url.host===location.host&&url.pathname.indexOf(PFX)===0){e.preventDefault();var file="";try{file=decodeURIComponent(url.pathname.slice(PFX.length));}catch(e2){file=url.pathname.slice(PFX.length);}if(!file)return;var tgt=(a.getAttribute("target")||"").toLowerCase();var newTab=!!(e.metaKey||e.ctrlKey||e.button===1||tgt==="_blank"||tgt==="_top");if(window.parent)window.parent.postMessage({type:"paw:open-canvas",path:file,newTab:newTab},"*");return;}e.preventDefault();if(window.parent)window.parent.postMessage({type:"paw:open-external",url:url.href},"*");},true);document.addEventListener("submit",function(e){var f=e.target;if(!f||f.tagName!=="FORM")return;var action=f.getAttribute("action")||"";if(action.indexOf("/api/forms/")!==0)return;e.preventDefault();var body={};new FormData(f).forEach(function(v,k){body[k]=v;});var btn=f.querySelector('button,[type=submit]');if(btn)btn.disabled=true;fetch(action,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)}).then(function(r){return r.json().catch(function(){return{ok:r.ok};});}).then(function(d){var ok=d&&d.ok;var m=document.createElement("div");m.textContent=ok?"\\u2713 Thanks! Your submission was received.":"Submission failed. Please try again.";m.style.cssText="margin-top:12px;padding:10px 14px;border-radius:8px;font:14px/1.4 system-ui,sans-serif;background:"+(ok?"#ecfdf5;color:#065f46":"#fef2f2;color:#991b1b");f.appendChild(m);if(ok)f.reset();if(btn)btn.disabled=false;}).catch(function(){if(btn)btn.disabled=false;});},true);})();</script>`;
