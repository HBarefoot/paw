...
interface HeartbeatPageProps {
  ...
  config: { enabled: boolean; intervalMinutes: number; triggerAiOnFailure: boolean };
  error?: string;
  success?: string;
}
...
export const HeartbeatPage: FC<HeartbeatPageProps> = ({ lastResult, history, config, error, success }) => {
  return (
    <Layout title="Heartbeat" currentPath="/heartbeat">
      ...
      <div class="flex gap-md" style="flex-wrap: wrap">
        ...
        <div class="card" style="flex: 2; min-width: 340px">
          <div class="flex justify-between items-center" style="margin-bottom: 12px">
            ...
            <button id="trigger-btn" class="btn-primary btn-sm" onclick="triggerCheck()">Run Check Now</button>
          </div>

          {lastResult ? (
            <div>
              <div class="flex gap-md items-center" style="margin-bottom: 16px">
                <span class={`badge ${lastResult.overallOk ? "success" : "error"}`} style="font-size: 13px; padding: 4px 12px">
                ...
                {lastResult.aiTriggered && (
                  <span class="badge neutral" style="font-size: 11px">AI Triggered</span>
                )}
              </div>
            ...
            <div class="empty-state">
              ...
              <p class="text-secondary" style="font-size: 13px; margin-top: 4px">Click "Run Check Now" to trigger one manually.</p>
            </div>
          )}
        </div>
        ...
        <div class="card" style="flex: 1; min-width: 260px">
          <h3>Configuration</h3>
          <form method="POST" action="/api/heartbeat/config" class="flex-col gap-md">
            <div>
              <label>Enabled</label>
              <select name="enabled" class="w-full">
                <option value="true" selected={config.enabled}>Yes</option>
                <option value="false" selected={!config.enabled}>No</option>
              </select>
            </div>
            <div>
              <label>Interval (minutes)</label>
              <input type="number" name="intervalMinutes" value={String(config.intervalMinutes)} min="1" max="1440" class="w-full" />
            </div>
            <div>
              <label>AI Investigation on Failure</label>
              <select name="triggerAiOnFailure" class="w-full">
                <option value="true" selected={config.triggerAiOnFailure}>Yes</option>
                <option value="false" selected={!config.triggerAiOnFailure}>No</option>
              </select>
            </div>
            <button type="submit" class="btn-primary self-start">Save</button>
          </form>
        </div>
      </div>
      ...
      <div class="card" style="margin-top: 16px">
        ...
        {history.length > 0 ? (
          <table>
            ...
            <tbody>
              {history.map((mem) => {
                ...
                return (
                  <tr>
                    <td style="white-space: nowrap">{formatTimestamp(mem.created_at)}</td>
                    <td>
                      <span class={`badge ${passed ? "success" : "error"}`}>
                        {passed ? "PASS" : "FAIL"}
                      </span>
                    </td>
                    <td class="text-secondary" style="font-size: 13px">{mem.text}</td>
                    <td>
                      <button class="btn-ghost btn-sm" onclick={`deleteMemory('${mem.id}')`}>
                        Delete
                      </button>
                    </td>
                  </tr>
          ...
          <div class="empty-state"><p>No heartbeat history yet.</p></div>
        )}
      </div>
