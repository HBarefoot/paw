...
export class WebAuthManager {
	...
	private totpAttempts: Map<string, { count: number; lockedUntil: number }> =
		new Map();
	...
	verifyAndEnableTotp(adminId: number, code: string): boolean {
		...
		this.db.run("UPDATE web_admins SET totp_verified = 1 WHERE id = ?", [
			adminId,
		]);
	...
	async login(
		...
		if (this.isTotpRequired(admin)) {
			...
			if (attempts && Date.now() < attempts.lockedUntil) {
				...
				return {
					success: false,
					error: "Too many TOTP attempts. Please wait 15 minutes.",
				};
			}

			if (!this.verifyTotpCode(admin, totpCode)) {
				// Track failed TOTP attempt
				const current = this.totpAttempts.get(username) ?? {
					count: 0,
					lockedUntil: 0,
				};
				current.count++;
